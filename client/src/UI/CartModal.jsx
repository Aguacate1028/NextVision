...
import { X, Minus, Plus, CreditCard, Trash2 } from 'lucide-react';

export const CartModal = ({ isOpen, onClose, cart, updateQuantity, removeFromCart, onConfirm, total }) => {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-end bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white h-full w-full max-w-md shadow-2xl relative flex flex-col animate-in slide-in-from-right duration-500">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.id_producto} className="bg-slate-50 rounded-3xl p-4 flex gap-4 items-center">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <button onClick={() => removeFromCart(item.id_producto)} className="text-red-400 hover:text-red-600"><Trash2 size={14}/></button>
                </div>
              </div>
            ))
          )}
        ...
        <footer className="p-8 bg-slate-50 border-t border-slate-100 rounded-t-[3rem] space-y-6">
          ...
          <button 
            ...
            onClick={onConfirm}
            className="w-full bg-blue-600 text-white py-6 rounded-[1.5rem] font-black uppercase tracking-[0.25em] shadow-2xl shadow-blue-200 hover:bg-blue-700 hover:-translate-y-1 transition-all flex items-center justify-center gap-3 disabled:bg-slate-300 disabled:translate-y-0"
          >
